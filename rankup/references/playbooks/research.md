...
| 4 | **哥飞 AI（SEO Agent）是必用的验证工具，不是可选步骤。** 调研结论涉及搜索量判断时，必须用 `gefei-ask.mjs` 向 SEO Agent 交叉验证关键词的月搜索量（Google Ads 级精度，2026-09-04 实测）。**派 sub agent 跑调研时，brief 里必须包含这一步的完整命令。** 它背后有无数知识库和工具可以使用，可以当成一个 subagent 使用——不只是查搜索量，还能审站、查竞品、做定性分析。**本机未配置 `SEO_WEBCAFE_COOKIE`，统一指定用 `gefei-ask.mjs`**（走 OpenCLI 驱动已登录 Chrome 问哥飞 SEO Agent，全程不碰凭据）；只有当会话 Cookie 已经存进 `SEO_WEBCAFE_COOKIE` 时，才用 `seo-webcafe.mjs chat` 的纯 HTTP 路径省去开浏览器。详见 [`seo-webcafe.md`](../seo-webcafe.md)「SEO Agent」与「两条取答路径怎么选」。 | Wave5 全程未使用交叉验证，导致搜索量数据只有 Semrush 单一来源，且多个小语种词 Semrush 返回 metrics_unavailable |
