...
| 闸门 4b · GEO 内容形状（AITDK GEO 标签页） | 每个内容页：≥1 条外部官方来源（`<cite>` + 外链）汇成 Sources 节；≥1 张规格表（`<table>`），半数以上行含有出处的数字；FAQ 问题是 H3；页面节点（SoftwareApplication / WebPage / WebSite）带 `author` + `datePublished` + `dateModified` 且页面有可见 `<time>`；Organization 不输出空 `sameAs` | `.rankup/evidence/aitdk-geo-<date>/` | **扩展那半截现在能自动跑**（2026-09-11 起）：`bash <rankup>/scripts/aitdk-opencli.sh <url> aitdk <out>.json`，读 `aitdkPanel.sections.geo.raw`（`fields` 对散文式的 GEO 是空的，别当没数据）。前置条件（AITDK 已登录、opencli 用仓库构建）与解析短板见 [`seo-box.md`](seo-box.md)「AITDK 面板全自动取数」；前置条件不满足才退回旧办法「请用户在 GEO 标签页跑一页贴回报告」。两条路都走不通时只做 curl 计数这半截：结果落 `.rankup/evidence/page-audit-<date>/geo-counts.tsv`，状态记 ⬜ 并注明原因，不许因为做不了另一半就跳过这一半。再 curl 全站数 `<table>/<blockquote>/<cite>/<h3>/<time>` 与 JSON-LD 字段逐页核。**先分「设计」与「缺口」**：robots 类三项在预览域恒 FAIL（`Disallow: /` 是故意的），段 5 放开索引后才算 | 动了页面 |
