 ...
 * 核实耗时且容易漏。免费/Basic 档位不能手动触发重抓（见 `ahrefs-site-audit.mjs`
 * 头部注释），下一次真正的抓取要等排程（`schedule` 子命令查得到），所以
 ...
 *
 * 已验证：2026-09-13（在真实项目的 11 类 issue 上人工核对过分类与判据的
 * 合理性；本脚本的自动化实现基于该次人工复核总结的判据，尚未在同一批
 * issue 上重跑本脚本做逐条比对——首次使用时建议抽样核对几条自动判定的
 * 结果，确认与人工复核结论一致）。
 */
